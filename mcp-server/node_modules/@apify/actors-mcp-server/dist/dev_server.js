...
!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="007a8434-4d6d-5c2d-80f7-4d5a6ddab415")}catch(e){}}();
var _a, _b;
var _c;
import { randomUUID } from 'node:crypto';
import { fileURLToPath } from 'node:url';
import { localhostHostValidation, localhostOriginValidation, toNodeHandler, toWebRequest, } from '@modelcontextprotocol/node';
...
(_a = (_c = process.env).TELEMETRY_ENV) !== null && _a !== void 0 ? _a : (_c.TELEMETRY_ENV = 'DEV');
/**
...
function extractApiTokenFromRequest(req) {
    ...
    if (typeof value === 'string') {
        ...
        if ((schema === null || schema === void 0 ? void 0 : schema.toLowerCase()) === 'bearer' && token)
            return token;
    }
    try {
        const tokenFromUrl = new URL((_a = req.url) !== null && _a !== void 0 ? _a : '', `http://${req.headers.host}`).searchParams.get('token');
 ...
 */
const UNAUTHORIZED_CHALLENGE = 'Bearer error="invalid_token", error_description="Apify API token missing. Create one at https://console.apify.com/settings/integrations"';
/**
 ...
 */
function resolveRequestAuth(req, res, paymentProvider) {
    if (paymentProvider)
        return { apifyToken: undefined };
 ...
 */
export async function isStatelessRequest(req) {
 ...
 */
async function serveStatelessRequest(req, res, taskStore) {
    var _a, _b;
    const urlParams = new URL(req.url, `http://${req.headers.host}`).searchParams;
    const telemetryEnabled = (_b = (_a = parseBooleanOrNull(urlParams.get('telemetry-enabled'))) !== null && _a !== void 0 ? _a : parseBooleanOrNull(process.env.TELEMETRY_ENABLED)) !== null && _b !== void 0 ? _b : true;
    ...
    const handler = createMcpHandler(async ({ requestInfo }) => {
        ...
        // authoritative here.
        const isDiscoverProbe = (requestInfo === null || requestInfo === void 0 ? void 0 : requestInfo.headers.get('mcp-method')) === 'server/discover';
        ...
        legacy: 'reject',
        onerror: (error) => log.softFail('Stateless MCP request rejected', { errMessage: sanitizeMezmoMessage(error.message) }),
    });
    try {
        ...
        if (apifyToken) {
            req.auth = { token: apifyToken, clientId: '', scopes: [] };
        }
    ...
    app.post('/', async (req, res) => {
        ...
        try {
            ...
            if (sessionId && transports[sessionId]) {
                ...
                const telemetryEnabled = (_b = (_a = parseBooleanOrNull(telemetryEnabledParam)) !== null && _a !== void 0 ? _a : parseBooleanOrNull(process.env.TELEMETRY_ENABLED)) !== null && _b !== void 0 ? _b : true;
                const uiParam = urlParams.get('ui');
                const serverMode = uiParam !== null ? parseServerMode(uiParam) : parseServerMode(process.env.UI_MODE);
                ...
                const requestOrigin = getRequestOriginForClient(buildMcpClientContext(parseInitializeParams(initializeMessage.params)));
 ...
 */
export function extractInitializeMessage(body) {
    const messages = Array.isArray(body) ? body : [body];
    return messages.find((msg) => typeof msg === 'object' && msg !== null && msg.method === 'initialize');
}
/**
 ...
 */
export function parseInitializeParams(params) {
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
    const HOST = (_b = process.env.HOST) !== null && _b !== void 0 ? _b : 'http://localhost';
...
//# debugId=007a8434-4d6d-5c2d-80f7-4d5a6ddab415
