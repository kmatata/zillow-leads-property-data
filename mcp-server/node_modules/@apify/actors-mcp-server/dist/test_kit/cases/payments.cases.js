...
!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="3a3ed790-f247-54a6-bd5f-78e7da1a8227")}catch(e){}}();
...
export const paymentsCases = [
        ...
        run: async (ctx) => {
            ...
            try {
                ...
                for (const toolName of skyfireEnabledToolNames) {
                    ...
                    const skyfireProperty = properties['skyfire-pay-id'];
                    ...
                    expect((_a = tool.description) === null || _a === void 0 ? void 0 : _a.includes('skyfire-pay-id'), `Tool "${toolName}" description should mention skyfire-pay-id`).toBe(true);
                }
            }
            finally {
                await client.close();
        ...
        run: async (ctx) => {
            ...
            try {
                ...
                for (const toolName of paidToolNames) {
                    const tool = toolsList.tools.find((t) => t.name === toolName);
                    expect(tool, `Tool "${toolName}" should exist in the tools list`).toBeDefined();
                    const x402 = (_a = tool === null || tool === void 0 ? void 0 : tool._meta) === null || _a === void 0 ? void 0 : _a.x402;
                    expect(x402, `Tool "${toolName}" should advertise _meta.x402`).toBeDefined();
                    expect(x402 === null || x402 === void 0 ? void 0 : x402.paymentRequired, `Tool "${toolName}" x402.paymentRequired should be true`).toBe(true);
                    for (const field of ['scheme', 'network', 'asset', 'payTo', 'amount']) {
                        expect(x402 === null || x402 === void 0 ? void 0 : x402[field], `Tool "${toolName}" should advertise x402.${field}`).toBeDefined();
                    }
                    const accepts = x402 === null || x402 === void 0 ? void 0 : x402.accepts;
                    expect(accepts, `Tool "${toolName}" should advertise x402.accepts[]`).toBeInstanceOf(Array);
                    expect(accepts === null || accepts === void 0 ? void 0 : accepts.length, `Tool "${toolName}" should advertise at least one accept entry`).toBeGreaterThan(0);
                    for (const entry of accepts !== null && accepts !== void 0 ? accepts : []) {
                ...
                for (const toolName of freeToolNames) {
                    const tool = toolsList.tools.find((t) => t.name === toolName);
                    expect(tool, `Tool "${toolName}" should exist in the tools list`).toBeDefined();
                    const meta = tool === null || tool === void 0 ? void 0 : tool._meta;
                    expect(meta === null || meta === void 0 ? void 0 : meta.x402, `Tool "${toolName}" should not advertise _meta.x402`).toBeUndefined();
                }
            }
            finally {
                await client.close();
    ...
    },
    {
        ...
        skipIf: skipOnStdio,
        run: async (ctx) => {
            const isProxiedAddTool = (name) => name.endsWith('-add');
            let client = await ctx.createClientFn({ payment: 'x402', actors: [ACTOR_EXAMPLE_MCP_SERVER] });
            try {
                const x402Tools = await client.listTools();
                expect(x402Tools.tools.filter((t) => isProxiedAddTool(t.name)), 'standby MCP-server sub-tools should not be loaded in x402 payment mode').toHaveLength(0);
                await client.close();
                client = await ctx.createClientFn({ payment: 'skyfire', actors: [ACTOR_EXAMPLE_MCP_SERVER] });
                const skyfireTools = await client.listTools();
                expect(skyfireTools.tools.filter((t) => isProxiedAddTool(t.name)), 'standby MCP-server sub-tools should not be loaded in skyfire payment mode').toHaveLength(0);
                await client.close();
                ...
                expect(normalTools.tools.filter((t) => isProxiedAddTool(t.name)), 'standby MCP-server sub-tools should be loaded under standard token auth').not.toHaveLength(0);
            }
            finally {
                await client.close();
            }
    ...
    },
    {
        ...
        run: async (ctx) => {
            ...
            const client = await ctx.createClientFn({ payment: 'x402' });
            try {
                const stream = client.experimental.tasks.callToolStream({ name: 'call-actor', arguments: { actor: ACTOR_EXAMPLE_MCP_SERVER, input: {} } }, CallToolResultSchema, { task: { ttl: 60000 } });
                ...
                for await (const message of stream) {
                    if (message.type === 'taskCreated') {
                        ...
                        resultIsError = message.result.isError;
                        const content = message.result.content;
                        resultText = (_a = content[0]) === null || _a === void 0 ? void 0 : _a.text;
                    }
    ...
    },
    {
        ...
        run: async (ctx) => {
            ...
            try {
                ...
                expect(structured === null || structured === void 0 ? void 0 : structured.x402Version, 'structuredContent.x402Version should be set').toBeDefined();
                expect(structured === null || structured === void 0 ? void 0 : structured.accepts, 'structuredContent.accepts should be an array').toBeInstanceOf(Array);
            }
            finally {
                await client.close();
            }
...
//# sourceMappingURL=payments.cases.js.map
//# debugId=3a3ed790-f247-54a6-bd5f-78e7da1a8227
