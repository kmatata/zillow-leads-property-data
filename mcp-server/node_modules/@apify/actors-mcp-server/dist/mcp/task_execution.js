...
!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="c86f276c-3f9c-5a9f-b1f9-0b74150f728c")}catch(e){}}();
...
import { createTaskCancellationWatcher, isTaskCancelled, isTaskNotFoundError, storeTaskResultOrSkipIfExpired, } from './utils.js';
...
function sendTaskProgressNotification(server) {
...
export async function emitTaskStatusNotification(taskId, mcpSessionId, taskStore, server) {
    try {
...
export async function executeToolAndUpdateTask(params) {
    const { taskId, tool, toolArgs, logSafeArgs, apifyClient, apifyToken, progressToken, mcpSessionId, actorName, actorId, clientContext, taskStore, server, tools, actorStore, paymentProvider, telemetryEnabled, telemetryEnv, transportType, sendNotification, } = params;
    const emitLog = async (msg) => server.sendLoggingMessage(msg);
    ...
    const finishTaskTracking = (status, diagnostics, result) => {
    ...
    const completeTask = async (storeStatus, taskResult, status, diagnostics) => {
    ...
    const skipIfTaskCancelled = async (logSuffix, status, diagnostics) => {
    ...
    try {
        ...
        const onStatusMessage = async (message) => {
        ...
        // and stops.
        const progressTracker = tool.type === TOOL_TYPE.ACTOR_MCP
            ? null
        ...
        if (await skipIfTaskCancelled(', skipping result storage', toolStatus))
            return;
        ...
        switch (errorResult.kind) {
            ...
            case TOOL_CALL_ERROR_KIND.EXECUTION: {
                ...
                // Check if task was cancelled before storing result
                if (await skipIfTaskCancelled(', skipping result storage', toolStatus, callDiagnostics))
                    return;
            ...
            default:
                ...
                throw new Error(`Unknown tool-call error kind "${errorResult.kind}"`);
        }
    }
    finally {
...
//# debugId=c86f276c-3f9c-5a9f-b1f9-0b74150f728c
