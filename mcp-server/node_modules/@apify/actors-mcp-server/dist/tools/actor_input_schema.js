...
!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="0a54c26f-0e23-5961-8880-09b4d738009e")}catch(e){}}();
...
import { addGlobsProperties, addKeyValueProperties, addProxyProperties, addPseudoUrlsProperties, addRequestListSourcesProperties, addResourcePickerProperties as addArrayResourcePickerProperties, } from '../utils/apify_properties.js';
...
export const MAX_UNTRUSTED_SCHEMA_BYTES = 262144;
// source https://github.com/ajv-validator/ajv/issues/1413#issuecomment-867064234
export function fixedAjvCompile(ajvInstance, schema) {
    ...
    ajvInstance.scope._values.schema.delete(schema);
    ajvInstance.scope._values.validate.delete(validate);
    const schemaIdx = ajvInstance.scope._scope.schema.indexOf(schema);
    const validateIdx = ajvInstance.scope._scope.validate.indexOf(validate);
    if (schemaIdx !== -1)
        ajvInstance.scope._scope.schema.splice(schemaIdx, 1);
    if (validateIdx !== -1)
        ajvInstance.scope._scope.validate.splice(validateIdx, 1);
 ...
 * @param properties
 */
export function inferArrayItemsTypeIfMissing(properties) {
    var _a, _b, _c;
    for (const [, property] of Object.entries(properties)) {
        if (property.type === 'array' && !((_a = property.items) === null || _a === void 0 ? void 0 : _a.type)) {
            ...
            if (itemsType) {
                property.items = {
                    ...property.items,
                    title: (_b = property.title) !== null && _b !== void 0 ? _b : 'Item',
                    description: (_c = property.description) !== null && _c !== void 0 ? _c : 'Item',
                    type: itemsType,
                };
            }
        }
 ...
 */
export function buildActorInputSchema(actorFullName, input, isRag) {
    if (!input) {
        return {
            inputSchema: {
 ...
 */
export function pruneSchemaPropertiesByWhitelist(input, whitelist) {
    if (!input || !input.properties || typeof input.properties !== 'object' || !whitelist)
        return input;
    const allowed = new Set(Array.from(whitelist));
    const newProps = {};
    for (const key of Object.keys(input.properties)) {
        if (allowed.has(key))
            newProps[key] = input.properties[key];
    }
    const cloned = { ...input, properties: newProps };
...
export function transformActorInputSchemaProperties(input) {
...
//# sourceMappingURL=actor_input_schema.js.map
//# debugId=0a54c26f-0e23-5961-8880-09b4d738009e
