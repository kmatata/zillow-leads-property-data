...
!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="67c15a53-995a-5d75-8b7a-ae8021ef123f")}catch(e){}}();
import { CallToolResultSchema } from '@modelcontextprotocol/sdk/types.js';
import dedent from 'dedent';
import { z } from 'zod';
import log from '@apify/log';
import { ApifyClient } from '../../apify_client.js';
import { APIFY_ERROR_TYPE_CANNOT_START_ACTOR_RUNS, APIFY_ERROR_TYPE_FULL_PERMISSION_NOT_APPROVED, APIFY_ERROR_TYPE_MEMORY_LIMIT_EXCEEDED, FAILURE_CATEGORY, HELPER_TOOLS, } from '../../const.js';
...
import { ACTOR_RUN_LIMIT_MESSAGE, isActorRunLimitError, isMemoryQuotaError, isPermissionApprovalError, remoteMcpFailureDetail, } from '../../utils/apify_errors.js';
import { getConsoleLinkContext } from '../../utils/console_link.js';
import { wrapJsonText } from '../../utils/encode_text.js';
import { logHttpError } from '../../utils/logging.js';
import { respondAborted, respondRaw, respondServerError, respondUserError, } from '../../utils/mcp.js';
...
export function buildCallActorMcpServerSection({ hasTool }) {
...
function buildCallActorUsageSection({ hasTool }) {
...
function buildActorNotFoundHint(loadedToolNames) {
    const hints = [
...
function buildWidgetAddendum({ hasTool }) {
    return dedent `
...
function buildCallActorDescriptionSections(includeWidget, ctx) {
    const { hasTool } = ctx;
    ...
    if (includeWidget)
        sections.push(buildWidgetAddendum(ctx));
    return sections.join('\n\n');
}
export function buildCallActorDescription(ctx = ALL_TOOLS_PRESENT) {
    return buildCallActorDescriptionSections(false, ctx);
}
export function buildCallActorAppsDescription(ctx = ALL_TOOLS_PRESENT) {
...
export function buildCallActorErrorResponse(params) {
    ...
    ].filter(Boolean), { error, detail: errMsg.slice(0, 200), actorId });
}
 ...
 */
export function resolveActorContext(actorName) {
...
export async function handleMcpToolCall(params) {
    const { baseActorName, mcpToolName, input, isActorMcpServer, mcpServerUrl, apifyToken, mcpSessionId, signal } = params;
    if (!isActorMcpServer) {
        return respondServerError(`Actor '${baseActorName}' is not an MCP server.`);
    }
    if (!input) {
        return respondServerError(`Input is required for MCP tool '${mcpToolName}'. Please provide the input parameter based on the tool's input schema.`);
    }
    if (signal.aborted) {
    ...
    try {
        ...
        return respondServerError(`Failed to call MCP tool '${mcpToolName}' on Actor '${baseActorName}': ${remoteMcpFailureDetail(error)}`);
    }
    finally {
        if (client)
            await client.close();
...
export async function resolveAndValidateActor(params) {
    ...
    if (!actor.ajvValidate(input)) {
        ...
        const ajvDetails = extractAjvErrorDetails(errors !== null && errors !== void 0 ? errors : null);
        const validationSummary = (_a = errors === null || errors === void 0 ? void 0 : errors.map((e) => e.message).join(', ')) !== null && _a !== void 0 ? _a : '';
 ...
 */
function createCallActorTool(buildDescription) {
...
export const callActorDefault = createCallActorTool(buildCallActorDescription);
/**
 * Apps mode call-actor tool.
 * Renders no widget; for a live progress UI, use the call-actor-widget sibling.
 */
export const callActorApps = createCallActorTool(buildCallActorAppsDescription);
//# sourceMappingURL=call_actor.js.map
//# debugId=67c15a53-995a-5d75-8b7a-ae8021ef123f
